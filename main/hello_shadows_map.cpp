...
namespace gl {
	...
	void HelloSkybox::Init()
	{
		...
		modele_ = std::make_unique<Model>(path + "data/mesh_test/damagedWall.obj");
	...
	{
		model_ = glm::rotate(glm::mat4(1.0f), 0.0f, glm::vec3(0.f, 1.f, 0.f));
	...
	void HelloSkybox::RenderScene(std::unique_ptr<Shader>& Shaders)
	{
		...
		inv_model_= glm::transpose(glm::inverse(model_));
		Shaders->SetMat4("model", model_);
		Shaders->SetMat4("inv_model", inv_model_);
		modele_->Draw(Shaders);

		model_ = glm::mat4(1.0f);
		model_ = glm::translate(model_, glm::vec3(0.0f, -15.0f, -35.0f));
		model_ = glm::scale(model_, glm::vec3(3.0f, 3.0f, 4.0f));
		//model_ = glm::rotate(model_, glm::radians(90.0f), glm::vec3(1.f, 0.f, 0.f));
		inv_model_ = glm::transpose(glm::inverse(model_));
		Shaders->SetMat4("model", model_);
		Shaders->SetMat4("inv_model", inv_model_);
		modele_->Draw(Shaders);
		...
		model_ = glm::mat4(1.0f);
		model_ = glm::translate(model_, glm::vec3(-25.0f, 0.0f, 0.0f));
		model_ = glm::rotate(model_, glm::radians(90.0f), glm::vec3(0.f, 1.f, 0.f));
		inv_model_ = glm::transpose(glm::inverse(model_));
		Shaders->SetMat4("model", model_);
		Shaders->SetMat4("inv_model", inv_model_);
		modele_->Draw(Shaders);
