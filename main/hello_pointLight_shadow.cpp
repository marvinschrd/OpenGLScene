...
namespace gl {
	...
	protected:
		...
		glm::vec3 light_position = glm::vec3(0.0, 1.0, 6.0);
		glm::vec3 light_Dir = glm::normalize(glm::vec3(0, 25, 100) - glm::vec3(0.0,0.0,0.0));
	...
	void HelloSkybox::Init()
	{
		...
		simpleDepthShaders_ = std::make_unique<Shader>(
			path + "data/shaders/hello_shadows/simplePointShadow.vert",
			path + "data/shaders/hello_shadows/simplePointShadow.frag",
			path + "data/shaders/hello_shadows/simplePointShadow.geom"
			);
		...
		//PointShadow depthFramebuffer
		glGenFramebuffers(1, &depthMapFBO);
		// create depth cubemap texture
		glGenTextures(1, &depthCubemap);
		glBindTexture(GL_TEXTURE_CUBE_MAP, depthCubemap);
		for (unsigned int i = 0; i < 6; ++i)
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_DEPTH_COMPONENT, shadowSizeWidth_, shadowSizeHeight_, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
		...
		generalShaders_ = std::make_unique<Shader>(
			path + "data/shaders/hello_shadows/pointShadow.vert",
			path + "data/shaders/hello_shadows/pointShadow.frag");
		generalShaders_->Use();
		generalShaders_->SetInt("depthMap", 4);
	...
	void HelloSkybox::Update(seconds dt, SDL_Window* window)
	{
		...
		glm::mat4 shadowProj = glm::perspective(glm::radians(90.0f), (float)shadowSizeWidth_ / (float)shadowSizeHeight_, near_plane, far_plane);
		...
		shadowTransforms.push_back(shadowProj * glm::lookAt(light_position, light_position + glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
		//Shadow pass
		glViewport(0, 0, shadowSizeWidth_, shadowSizeHeight_);
		glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
		glClear(GL_DEPTH_BUFFER_BIT);
		simpleDepthShaders_->Use();
		for (unsigned int i = 0; i < 6; ++i)
		{
			simpleDepthShaders_->SetMat4("shadowMatrices[" + std::to_string(i) + "]", shadowTransforms[i]);
			simpleDepthShaders_->SetFloat("far_plane", far_plane);
			simpleDepthShaders_->SetVec3("lightPos", light_position);
		}
			RenderScene(simpleDepthShaders_);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		//
		...
		generalShaders_->SetMat4("projection", projection_);
		generalShaders_->SetVec3("cameraPosition", camera_->position);
		generalShaders_->SetVec3("lightPos", light_position);
		generalShaders_->SetFloat("far_plane", far_plane);
		generalShaders_->SetVec3("lightDirection", light_Dir);

		
		generalShaders_->SetFloat("constant", 1.0f);
		generalShaders_->SetFloat("linear", 0.09f);
		generalShaders_->SetFloat("quadratic", 0.032f);
